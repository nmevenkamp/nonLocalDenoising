...
protected:
  RealType getStepSizeWithSelectedController( DerivativeType &DescentDir,
...
public:
  void writeEnergy( const RealType CurrentEnergy ) const{
    if( _out != NULL )
...
public:
  ...
  virtual void applySingle( VectorType &Dest ) const{
    ...
    DerivativeType oldDirection;
    DerivativeType oldGradient;
    // Only allocate memory for the nonlin CG storage variables if necessary.
    if ( _configurationFlags & USE_NONLINEAR_CG ) {
      oldDirection.reallocate ( Dest );
      oldGradient.reallocate ( Dest );
    }
