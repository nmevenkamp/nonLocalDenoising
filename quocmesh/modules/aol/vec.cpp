...
aol::Vector<_DataType>::Vector ( const std::vector<DataType> & std_vec )
    ...
    overflowHandling ( aol::CLIP ), overflowMin ( 0 ), overflowMax ( aol::OverflowTrait<DataType>::max ) {
  throw aol::Exception ( "FIXME", __FILE__, __LINE__ );
  _pData = static_cast<DataType*> ( aol::MemoryManager::allocateAtLeast ( _size, sizeof ( DataType ) ) ); // this should probably be _sizeReserved because of what allocateAtLeast does to its argument.
