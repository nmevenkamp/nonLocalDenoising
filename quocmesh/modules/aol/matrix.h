...
public:
  ...
  SparseBlockMatrix () : BlockOpBase<typename MatrixType::DataType, MatrixType> ( 0, 0 ), _deleteFlag ( 0 ) {
  }

  SparseBlockMatrix ( const int M, const int N ) : BlockOpBase<typename MatrixType::DataType, MatrixType> ( M, N ), _deleteFlag ( M * N ) {
  ...
  : BlockOpBase<typename MatrixType::DataType, MatrixType> ( Matrix.getNumRows(), Matrix.getNumCols() ), _deleteFlag ( Matrix.getNumRows() * Matrix.getNumCols() ) {
    switch ( copyFlag ) {
      case STRUCT_COPY: {
  ...
  SparseBlockMatrix<MatrixType>& operator*= ( const typename MatrixType::DataType alpha ) {
    for ( int i = 0; i < this->getNumRows (); ++i ) {
      for ( int j = 0; j < this->getNumCols (); ++j ) {
        if ( this->getPointer ( i, j ) )
   ...
   *
   * Assumes that all block matrices are of the same size.
   */
  void addUnblockedMatrixTo ( Matrix<RealType> &BigMatrix ) const {
    ...
    const int blockEntryRows = tempPointer->getNumRows();
    const int blockEntryCols = tempPointer->getNumCols();

    BigMatrix.resize ( this->getNumRows() * blockEntryRows, this->getNumCols() * blockEntryCols );
...
public:
  ...
  // obviously similar method getTotalNumCols not implemented yet.
