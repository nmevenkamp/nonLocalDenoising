 ...
 * \author Berkels
 */
template <typename RealType, typename WrappedOpType, typename VecType = aol::Vector<RealType> >
class DerivativeWrapper : public aol::Op<VecType > {
  const WrappedOpType &_op;
public:
  DerivativeWrapper ( const WrappedOpType &Op ) : _op ( Op ) {}

  void applyAdd ( const VecType &Arg, VecType &Dest ) const {
    VecType result ( Dest, aol::STRUCT_COPY );
    apply ( Arg, result );
    Dest += result;
  }
