...
class GaussNewtonAlgorithm : public ArmijoLineSearchUsingOp<RealType, aol::Vector<RealType> > {
  ...
  const aol::Op<aol::Vector<RealType>, MatrixType> &_DF;
  const int _maxIterations;
  const RealType _dampingFactor;

  const StepSaverBase<RealType, aol::Vector<RealType> > *_pStepSaver;

  RealType ArmijoLineSearchHelpFunction_evaluate( const aol::Vector<RealType> &DescentDir, const aol::Vector<RealType> &CurrentPosition, const RealType timestepWidth ) const {
    aol::Vector<RealType> temp ( CurrentPosition );
    temp.addMultiple ( DescentDir, timestepWidth );
    aol::Vector<RealType> f ( _pMatDF->getNumRows() );
    _F.apply ( temp, f );
    return f.normSqr();
  }
  
  // Attention: ArmijoLineSearchHelpFunction_evaluateDerivative only works, if _pMatDF is filled with _DF(Position)!
  RealType ArmijoLineSearchHelpFunction_evaluateDerivative( const aol::Vector<RealType> &DescentDir, const aol::Vector<RealType> &Position ) const {
    aol::Vector<RealType> tmp ( _pMatDF->getNumRows() );
    aol::Vector<RealType> tmp2 ( _pMatDF->getNumRows() );
...
public:
  GaussNewtonAlgorithm ( const int DimRangeF,
                         const aol::Op<aol::Vector<RealType> > &F,
                         const aol::Op<aol::Vector<RealType>, MatrixType> &DF,
                         const int MaxIterations = 50,
                         const RealType DampingFactor = 0 )
    : ArmijoLineSearchUsingOp<RealType, aol::Vector<RealType> > ( 0.5, MaxIterations ),
      _pMatDF ( NULL ),
      _dimRangeF ( DimRangeF ),
      _F ( F ),
      _DF ( DF ),
  ...
  void apply( const aol::Vector<RealType> &Arg, aol::Vector<RealType> &Dest ) const {
    const int numRows = _dimRangeF + ( ( _dampingFactor > 0 ) ? Arg.size() : 0 );
    const int numCols = Arg.size();

    if ( _pMatDF.get() == NULL )
      _pMatDF.reset ( new MatrixType ( numRows, numCols ), true );

    if ( _dampingFactor > 0 ) {
      for ( int i = 0; i < Arg.size(); ++i )
        _pMatDF->set ( _dimRangeF + i, i, 1 );
    }
...
#ifdef USE_EXTERNAL_EIGEN
    Eigen::Matrix<RealType, Eigen::Dynamic, Eigen::Dynamic> A ( numRows, numCols );
    Eigen::Matrix<RealType, Eigen::Dynamic, 1> rhs ( numRows );
    Eigen::Matrix<RealType, Eigen::Dynamic, 1> sol ( numCols );
    aol::Vector<RealType> f ( rhs.data(), numRows, aol::FLAT_COPY );
    aol::Vector<RealType> direction ( sol.data(), numCols, aol::FLAT_COPY );
#else
    ...
    do {
      ...
      cerr << "Step " << iteration << ", tau " << tau << ", fNormSqr " << fNormSqr << endl;

      const bool fNormDecreased = ( fNormSqr < fNormSqrOld );
  ...
  void setStepSaverReference ( const StepSaverBase<RealType, aol::Vector<RealType> > &StepSaver ) {
