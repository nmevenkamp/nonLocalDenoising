...
class FELeastSquaresFunctionalInterface{
public:
  typedef typename ConfiguratorType::RealType RealType;
protected:
  const typename ConfiguratorType::InitType &_initializer;
...
public:
  explicit FELeastSquaresFunctionalInterface ( const typename ConfiguratorType::InitType &Initializer ) :
    _initializer ( Initializer ), _config(Initializer) {
  }
  
  FELeastSquaresFunctionalInterface ( const ConfiguratorType & Config, const typename ConfiguratorType::InitType &Initializer ) :  _initializer ( Initializer ), _config(Initializer) {
  }
  ...
  void apply ( const aol::Vector<RealType> &Arg, aol::Vector<RealType> &Dest ) const {
    ...
    const aol::DiscreteFunctionDefault<ConfiguratorType> discFunc ( _config, Arg );
    
    const typename IteratorType::EndType end = _config.end();
    int i = 0;
    for ( IteratorType it = _config.begin(); it != end; ++it ) {
      const typename ConfiguratorType::BaseFuncSetType &bfs = _config.getBaseFunctionSet ( *it );
      const int numQuadPoints = bfs.numQuadPoints( );
      aol::Vector<RealType> a(numQuadPoints);
      for ( int q = 0; q < numQuadPoints; ++q ) {
        //i+q = numQuadPoints * _config.getConsecutive
        Dest[i+q] = this->asImp().evaluateIntegrand( discFunc, *it, q, bfs.getRefCoord ( q ) ) * sqrt( bfs.getWeight ( q ) );
      }
      i+=numQuadPoints;
    }
  }
  ...
  void applyMultBasisFunction ( const aol::Vector<RealType> &Arg, MatrixType &Dest ) const {
    ...
    for ( IteratorType it = _config.begin(); it != end_it; ++it ) {
      ...
      for ( int dof = 0; dof < numLocalDofs; dof++ )
        for ( int q = 0; q < numQuadPoints; ++q )
          Dest.set(i+q, _config.localToGlobal ( *it, dof ), integrandWithoutBasisFunction[i+q] * bfs.evaluate ( dof, q ) );
      i+=numQuadPoints;
    }
  }
  ...
  void applyMultDerBasisFunction ( const aol::Vector<RealType> &Arg, int gradComp, MatrixType &Dest ) const {
    ...
    for ( IteratorType it = _config.begin(); it != end_it; ++it ) {
      ...
      for ( int dof = 0; dof < numLocalDofs; dof++ )
        for ( int q = 0; q < numQuadPoints; ++q )
          Dest.set(i+q, _config.localToGlobal ( *it, dof ),
