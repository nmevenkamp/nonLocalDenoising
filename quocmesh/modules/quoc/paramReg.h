...
public:
  ...
  void descentOnCurrentGrid( ) {
    ...
    if( _saveSteps )
      saveCurrentDeformation ( "_before", false );
...
void computeSeriesCropParameters ( const typename ConfiguratorType::InitType &Grid,
  ...
  aol::RandomAccessContainer<aol::Vec2<RealType> > centers ( numImages, aol::Vec2<RealType> ( ( minX + maxX ) / 2, ( minY + maxY ) / 2 ) );
  for ( int i = 0; i < numImages-1; ++i ) {
    ...
    cropWidth = static_cast<RealType>( CropSizeX ) / 2.;
      ...
      for ( int i = 0; i < numImages; ++i ) {
        for ( int j = 0; j < 2; ++j ) {
          ...
          maxCenter[j] = aol::Max ( minCenter[j], aol::Rint( centers[i][j] ) );
        }
      }
