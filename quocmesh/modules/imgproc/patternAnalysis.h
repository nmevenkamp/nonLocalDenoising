...
RealType periodicityEnergy1D ( const PictureType &Data, const RealType &Angle, const RealType Period ) {
  ...
  for ( int y=0; y<Data.getNumY ( ) ; ++y ) {
    for ( int x=0; x<Data.getNumX ( ) ; ++x ) {
      shiftedPos.set ( x + Period * cos ( Angle ), y + Period * sin ( Angle ) );
      if ( shiftedPos[0] >= 0 && shiftedPos[0] < Data.getNumX ( ) && shiftedPos[1] >= 0 && shiftedPos[1] < Data.getNumY ( ) && aol::isFinite<RealType> ( Data.get ( x, y ) ) && aol::isFinite<RealType> ( Data.interpolate ( shiftedPos ) ) ) {
        energy += aol::Sqr<RealType> ( Data.get ( x, y ) - Data.interpolate ( shiftedPos ) );
        ++numPoints;
      }
    }
  }
